...
async def warn(c: Sflix, m: Message):
...
@Sflix.on_message(command("resetwarns") & restrict_filter)
async def reset_warn(c: Sflix, m: Message):
...
@Sflix.on_message(command("warns") & filters.group)
async def list_warns(c: Sflix, m: Message):
...
async def remove_warn(c: Sflix, m: Message):
...
@Sflix.on_callback_query(filters.regex("^warn."))
async def remove_last_warn_btn(c: Sflix, q: CallbackQuery):
...
@Sflix.on_message(command(["warnings", "warnsettings"]) & admin_filter)
...
@Sflix.on_message(command("warnmode") & admin_filter)
...
@Sflix.on_message(command("warnlimit") & admin_filter)
