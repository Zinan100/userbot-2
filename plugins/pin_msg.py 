...
from plugins.utils.string import build_keyboard, parse_button

SUPPORT_GROUP = ("TameSflix")

@Sflix.on_message(command("pin") & admin_filter)
async def pin_message(_, m: Message):
    ...
    if m.reply_to_message:
        try:
            ...
            await m.reply_text(
                ("pin.pinned_msg").format(message_link=message_link),
            ...
            await m.reply_text("pin.no_rights_pin")
...
@Sflix.on_message(command("unpin") & admin_filter)
async def unpin_message(c: Sflix, m: Message):
    try:
        if m.reply_to_message:
            ...
            await m.reply_text("pin.unpinned_last_msg")
        else:
            await c.unpin_chat_message(m.chat.id)
            await m.reply_text("Unpinned last pinned message!")
    except ChatAdminRequired:
        await m.reply_text("admin.not_admin")
    except RightForbidden:
        await m.reply_text("pin.no_rights_unpin")
    except RPCError as ef:
        await m.reply_text(
            ("general.some_error").format(
                SUPPORT_GROUP=SUPPORT_GROUP,
...
@Sflix.on_message(command("unpinall") & admin_filter)
...
@Sflix.on_callback_query(regex("^unpin_all_in_this_chat$"))
async def unpinall_calllback(c: Sflix, q: CallbackQuery):
...
@Sflix.on_message(command("antichannelpin") & admin_filter)
async def anti_channel_pin(_, m: Message):
    ...
    if len(m.text.split()) == 1:
        ...
        await m.reply_text(
            ("pin.antichannelpin.current_status").format(
                status=status,
            ),
        )
        return
    ...
    if len(m.text.split()) == 2:
        if m.command[1] in ("yes", "on", "true"):
            ...
            msg = ("pin.antichannelpin.turned_on")
            ...
            msg = ("pin.antichannelpin.turned_off")
        else:
            await m.reply_text("general.check_help")
            return
...
@Sflix.on_message(command("pinned") & admin_filter)
async def pinned_message(c: Sflix, m: Message):
...
@Sflix.on_message(command("cleanlinked") & admin_filter)
...
@Sflix.on_message(command("permapin") & admin_filter)
